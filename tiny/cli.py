...
def analyze(
    ...
    input_file: Path | None = typer.Option(
        None, "--input", "-i", help="Input file (FASTA, FASTQ, GenBank, EMBL)"
    ),
    output: Path | None = typer.Option(None, "--output", "-o", help="Output file for results"),
    format_info: bool = typer.Option(
        False, "--format-info", help="Show additional format-specific information"
    ),
    feature_limit: int = typer.Option(
        5, "--feature-limit", "-fl", help="Number of features to display per type (0 for all)"
    ),
    feature_type: str = typer.Option(
        None, "--feature-type", "-ft", help="Filter features by type (e.g., CDS, gene, tRNA)"
    ),
    save_features: bool = typer.Option(
        False, "--save-features", "-sf", help="Save complete feature information to a separate file"
    ),
):
            ...
            if input_file:
                ...
                if format_info and any(r.features for r in records):
                    ...
                    for feat_type, features in sorted(feature_types.items()):
                        ...
                        display_features = (
                            features if feature_limit == 0 else features[:feature_limit]
                        )
                    ...
                    if save_features:
                        features_output = (
                            output.parent / f"{output.stem}_features.json"
                            if output
...
def compare(
    ...
    input_file: Path | None = typer.Option(
        None, "--input", "-i", help="Input file containing sequences to compare"
    ),
):
    ...
    try:
        with show_progress("Comparing sequences") as progress:
            ...
            console.print(
                Panel(results_table, title="[bold cyan]Sequence Comparison", border_style="blue")
            )
            ...
            if comparison.mutations:
                ...
                for mut in comparison.mutations:
                    mutations_table.add_row(str(mut.position), mut.original, mut.mutated)
...
def align(
    ...
    try:
        with show_progress("Performing alignment") as progress:
            ...
            console.print(
                Panel(stats_table, title="[bold cyan]Alignment Statistics", border_style="blue")
            )
